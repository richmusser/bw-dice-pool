...
async function resourcesRollCallback(dialogHtml, stat, actor) {
    ...
    const callons = actor.getCallons("resources").map(s => {
        return { label: s, ...buildRerollData({ actor, roll, accessor: "resources" }) };
    });
    ...
    if (!isSuccess) {
        ...
        const taxMessage = new Dialog({
            ...
            content: `<p>${game.i18n.localize('BW.dialog.failResourcesText1')
                .replace("{diff}", rollData.difficultyGroup)}</p><p>${game.i18n.localize('BW.dialog.failResourcesText2')}</p><hr/>`,
            buttons: {
                full: {
