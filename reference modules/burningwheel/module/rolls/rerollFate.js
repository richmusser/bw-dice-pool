...
export async function handleFateReroll(target) {
    ...
    if (actor.system.fate === 0) {
        return helpers.notifyError(game.i18n.localize('BW.roll.noFateTitle'), game.i18n.localize('BW.roll.noFateMessage'));
    }
    let rollStat;
                ...
                if (learningTarget === 'skill') {
                    ...
                    if (successes <= obstacleTotal && success) {
                        if (actor.successOnlyRolls.includes(learningTarget)) {
                            actor.addStatTest(getProperty(actor, `system.${learningTarget}`), learningTarget.titleCase(), `system.${learningTarget}`, target.dataset.difficultyGroup, true);
                        }
                    }
